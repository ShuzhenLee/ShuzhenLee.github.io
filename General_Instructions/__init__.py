...
class General_Instructions(Page):
    def before_next_page(player, timeout_happened):
        if 'treatment' not in player.participant.vars:
            ...
            if session_name == 'Treatment_1_Baseline_Descriptive_FLEX_Sustainable':
                treatment = 'T1'
            ...
            elif session_name == 'Treatment_2_Demo_Account_FLEX_Sustainable':
                treatment = 'T2'
                player.participant.vars['treatment'] = treatment
                player.treatment = treatment
                logging.info(f"Participant {player.participant.code} assigned to treatment: {treatment}")
            elif session_name == 'Treatment_3_Descriptive_FLEX_Normal_Labelling':
                treatment = 'T3'
                player.participant.vars['treatment'] = treatment
                player.treatment = treatment
                logging.info(f"Participant {player.participant.code} assigned to treatment: {treatment}")
            elif session_name == 'Treatment_4_Demo_Account_FLEX_Normal_Labelling':
                treatment = 'T4'
                player.participant.vars['treatment'] = treatment
                player.treatment = treatment
                logging.info(f"Participant {player.participant.code} assigned to treatment: {treatment}")
...
class Green_Charity(Page):
    ...
    @staticmethod
    def is_displayed(player):
        # Only show if treatment is T1 or T2
        return player.participant.vars.get('treatment') in ['T1', 'T2']
