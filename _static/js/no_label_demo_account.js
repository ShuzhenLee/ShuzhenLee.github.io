document.addEventListener('DOMContentLoaded', (function() {
    ...
    function initializeNewChart() {
        ...
        for (let i = 0; i < minPeriods - 1; i++) {
            flexChart.series[0].addPoint({ x: i, y: chartData.conData[i] }, false, false);
            flexChart.series[1].addPoint({ x: i, y: chartData.susData[i] }, false, false);
    ...
    function addPeriodData() {
        if (newIndex < chartData.susData.length && newIndex <= maxPeriods) {
            flexChart.series[0].addPoint({ x: newIndex, y: chartData.conData[newIndex] }, false, false);
            flexChart.series[1].addPoint({ x: newIndex, y: chartData.susData[newIndex] }, false, false);
